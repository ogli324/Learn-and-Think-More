...
url：https://gtoad.github.io/2018/09/20/Android-Native-Hook-Practice-Arm64/
...
1. 当某个通用寄存器之后马上就要被赋值的情况下，也可以和`优化1`中的优化方案一样，因为此时尽管该寄存器之后会被使用，但是它在Hook处的值对之后的程序运行无影响，因此可以不用保存与恢复。
